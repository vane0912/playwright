...
test('Processing speeds appear and work', async({page}) => {
    ...
    let sidebar_validations = ['India Tourist eVisa', '1 Traveler', '+ Government fees', '$ 41.00', '+ Standard, 5 days', '$ 79.99']
    ...
    await expect(correct_total).toHaveText('120.99')

    await rush.click()
    await page.waitForTimeout(3000)
    await expect(continue_sidebar).toBeEnabled()

    sidebar_validations = ['India Tourist eVisa', '1 Traveler', '+ Government fees', '$ 41.00', '+ Rush, 3 days', '$ 109.99']
    sidebar_validations.forEach(async txt => await expect(sidebar_step_2).toContainText(txt))
    await expect(correct_total).toHaveText('150.99')

    await super_rush.click()
    await page.waitForTimeout(3000)
    await expect(continue_sidebar).toBeEnabled()

    sidebar_validations = ['India Tourist eVisa', '1 Traveler', '+ Government fees', '$ 41.00', '+ Super Rush, 2 days', '$ 169.99']
    sidebar_validations.forEach(async txt => await expect(sidebar_step_2).toContainText(txt))
    await expect(correct_total).toHaveText('210.99')
    await page.waitForTimeout(3000)
