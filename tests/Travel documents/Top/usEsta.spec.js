...
test('United States ESTA', async ({ page }) => {
  test.slow()
  await appFunctions.step_1(page,"au", "usa/apply-now")
  const continue_sidebar = page.locator('id=btnContinueSidebar')

  await appFunctions.step_2(page,continue_sidebar)
  await page.waitForURL("**/usa/apply-now#step=step_3c")
  ...
  await selectors.booleanOptions(page, 'general.hotel_info_us', 'boolean-No')
  await page.waitForTimeout(2000)
  await expect(next_btn).toBeEnabled()
  await next_btn.click()

  await page.waitForURL(deploy_url + "order/" + Order_num + "/continue#step=emergency_contact")
  await selectors.booleanOptions(page, 'general.emergency_contact_us', 'boolean-No')
  await expect(next_btn).toBeEnabled()
  await next_btn.click()
  ...
  await page.waitForTimeout(2000)
  await selectors.booleanOptions(page, 'applicant.0.are_employed', 'boolean-Employed (including self-employed, freelancers, and business owners)')
  await selectors.inputText(page, 'applicant.0.employer_name', 'Test')
  await selectors.booleanOptions(page, 'applicant.0.employer_info_us', 'boolean-No')
  await page.waitForTimeout(2000)
  await expect(next_btn).toBeEnabled()
  await next_btn.click()
